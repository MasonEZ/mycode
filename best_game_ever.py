#!/usr/bin/env python3
# Hangman game. Enjoy!
# from my_brain_and_interwebs import the_most_ridiculous_code_this_side_of_the_mississippi_(West)
...
def howdy():  # You have to say it like spongebob mocking sandy ("Howdy YALl!")
    ...
    while True:
        gamechoice = input("Yes or No, Player? [y/n] >>:  \t\n").upper().strip()
...
def welcome(): # to my tape  2nd welcome message / word parameters
    print("The word You need to guess has", answer_length, "letters.")
    print("You can only enter 1 letter at a time.\n")


def wrong_letter(wrong): # saves and displays guesses that are... um... wrong.
...
# Fruitless loops main game loop
def gameloop(answer):
    ...
    while True:
        ...
        if not guess.isalpha(): # not recognized entry
            print("Please enter [a-z] only.")
            continue
        if guess in answer: # correct guess stuff
            ...
            wrong_letter(wrong)
            if "".join(guess_word) == answer: # win parameters
                return "Congrats or whatever... You did IT!!! *sarcastic woo*\n"
        else:
            tries = tries + 1
            if tries == 6: # lose parameters
                wrong_letter(wrong)
                ...
                print('   ', '|   / \\')
                print('------------')
                return "'Sorry'! kind of... Game Over.\n"
            print("=" * 50) # good ol' wrong guess section around these parts
...
if __name__ == "__main__":  # Run it back!
    howdy()
    welcome()
    while True:
